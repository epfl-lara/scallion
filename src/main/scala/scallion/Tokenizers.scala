...
/** Contains definitions for tokenizers. */
trait Tokenizers extends Sources with RegExps {
  
  //---- Abstract members ----//

  /** Type of tokens generated by the tokenizer. */
  type Token

  /** The error token.
   *
   * Automatically produced by the tokenizer when it fails.
   */
   ...
   *
   *
   * Automatically produced by the tokenizer when it reaches the end of the source. */
  val EndToken: Token

  /** Returns canonic representation of tokens. */
  def represent(token: Token): Seq[Character]
  ...
  object Tokenizer {
    ...
    /** Creates a tokenizer for a sequence of producers.
     *
     * @param producers The producers, in decreasing priority.
     */
    def apply(producers: Producer*): Tokenizer = new Tokenizer(producers)
  }
}